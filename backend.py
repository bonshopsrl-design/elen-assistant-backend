...
def price_lookup(body: PriceLookupIn):
    ...
    MAX_PAGES = int(os.getenv("LOOKUP_MAX_PAGES", "50"))    # 可按店铺体量调大；扫到底也会自动停止
    ...
    # ===== 3) 大量商品的“分页兜底包含匹配”（since_id 分页，直到结束或达到 MAX_PAGES）=====
    since_id = 0
    pages = 0
    while pages < MAX_PAGES:
        ...
        for p in page:
            ...
            if not v_list:
                continue
            # 用第一个变体代表这个商品（也可以把所有变体都 push，但通常以商品为粒度即可）
            v = v_list[0]
            t_l = (p.get("title") or "").lower()
            s_l = (v.get("sku") or "").lower()
        ...
        # 小优化：有足够结果且都很优，就可以收手
        if len(items) >= limit:
            best_score = min(it["_score"] for it in items)
            if best_score <= 0.5:
                break
